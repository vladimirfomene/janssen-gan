...
def separate_data(data_loader):
    ...
    return negative_data.resize_((negative_counter, 10, 3, 28, 28)), positive_data.resize_(positive_counter, 10, 3, 28,
                                                                                           28)
...
for epoch in range(num_epochs):
    for n_batch, batch in enumerate(train_positives):

        # 1. Train Discriminator
        X_batch = images_to_vectors(batch.permute(0, 2, 3, 1).numpy())

        Z_batch = images_to_vectors(train_negatives[n_batch].permute(0, 2, 3, 1).numpy())
